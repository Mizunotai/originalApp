...
//        //7個目の座標
    ...
    home =[[UIButton alloc]initWithFrame:CGRectMake(self.view.frame.size.width/2 - 30, 268,80, 80)];
...
-(void)playtime:(NSTimer *)time{
    ...
    if (Playtime ==0){
        ...
        [Endbtn addTarget:self
                   action:@selector(backHome:) forControlEvents:UIControlEventTouchUpInside];
    ...
    if (count[0] == 3 && count[1] == 3 && count[2] == 3 && count[3] == 3 && count[4] ==3) {
        ...
        mainBidama.userInteractionEnabled = NO;
         ...
         clearcount =4;
       [countUD setInteger:clearcount forKey:@"key"];//クリア用の数を保存
         NSLog(@"%@保存成功 保存世の数%d",[[[NSUserDefaults standardUserDefaults]dictionaryRepresentation]allKeys],clearcount);
    }
    /*レベル４*/
...
-(IBAction)stop{
    ...
    mainBidama.userInteractionEnabled= NO;
    //スットプ時ホームボタンを表示する
    UIImage *homeimg =[UIImage imageNamed:@"HomeImg.png"];
